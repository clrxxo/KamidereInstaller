...
func init() {
	...
	if sudoUser != "" {
		if sudoUser == "root" {
			panic("Kamidere Installer must not be run as the root user. Please rerun as a normal user. Use sudo or doas only when prompted.")
		}

		Log.Debug("Kamidere Installer was run with root privileges, actual user is", sudoUser)
		...
		panic("Kamidere Installer was run as root but neither SUDO_USER nor DOAS_USER are set. Please rerun me as a normal user, with sudo/doas, or manually set SUDO_USER to your username")
	}
	Home = os.Getenv("HOME")
