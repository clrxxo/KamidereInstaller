...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Kamidere")
	var updateFlag = flag.Bool("repair", false, "Repair Kamidere")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Kamidere")
	...
	if *versionFlag {
		fmt.Println("KamidereCli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2026 Kamidere contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Kamidere Installer' option, or run with --update-self")
			}
		}()
		...
		case "Quit":
			return
		case "Update Kamidere Installer":
			if err := UpdateSelf(); err != nil {
				Log.Error("Failed to update self:", err)
				exitFailure()
			}
	...
	if install {
		...
		Log.Info("Downloading latest Kamidere files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise,", BrandName, "will likely not work.")
}
