...
func InitGithubDownloader() {
	...
	IsDevInstall = getenvAny(EnvDevInstall, LegacyEnvDevInstall) == "1" || buildinfo.InstallerTag == buildinfo.VersionUnknown
	Log.Debug("Is Dev Install: ", IsDevInstall)
	if buildinfo.InstallerTag == buildinfo.VersionUnknown {
		Log.Debug("Treating local non-release installer build as a dev install")
	}
	if IsDevInstall {
		GithubDoneChan <- true
		return
	...
	kamidereFile := KamidereDirectory

	stat, err := os.Stat(kamidereFile)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		kamidereFile = path.Join(kamidereFile, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(kamidereFile)
	if err != nil {
		return
	}

	Log.Debug("Found existing", BrandName, "install. Checking for hash...")

	re := regexp.MustCompile(`// (?:Kamidere|Equicord) (\w+)`)
...
func installLatestBuilds() (retErr error) {
	...
	out, err := os.OpenFile(KamidereDirectory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", KamidereDirectory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", KamidereDirectory+":", err)
		retErr = err
		return
	}
	...
	_ = FixOwnership(KamidereDirectory)
