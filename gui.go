...
func main() {
	...
	win = g.NewMasterWindow(InstallerDisplayName, 1200, 800, linuxFlags)
...
func InstallLatestBuilds() (err error) {
	...
	if err != nil {
		ShowModal("Uh Oh!", "Failed to install the latest Kamidere builds from GitHub:\n"+err.Error())
	}
	return
}
						...
						g.Style().SetFontSize(20).To(
							g.Label(
								"Would you like to update now?\n\n"+
									...
									"On macOS, auto updates are not supported, so it will instead open in your browser.",
							),
						),
						g.Row(
							g.Button("Update Now").
								OnClick(func() {
									if runtime.GOOS == "darwin" {
										...
										if err != nil {
											ShowModal("Failed to resolve download", err.Error())
											return
										}
										g.OpenURL(url)
										return
...
func renderInstaller() g.Widget {
	...
	layout := g.Layout{
		...
		g.Style().SetFontSize(20).To(
			renderErrorCard(
				DiscordYellow,
				"Only official Kamidere release locations should be trusted. Any other site claiming to distribute Kamidere is potentially malicious.\n"+
				...
				g.Style().
					...
					To(
						g.Button("Reinstall / Repair").
							...
							Size((w-40)/4, 50),
						Tooltip("Reinstall & Update Kamidere"),
					),
				g.Style().
		...
		InfoModal("#patched", "Successfully Patched", "If Discord is still open, fully close it first.\n"+
			"Then, start it and verify Kamidere installed successfully by looking for its category in Discord Settings"),
		...
		InfoModal("#scuffed-install", "Hold On!", "You have a broken Discord Install.\n"+
			...
			"You need to fix this before patching, otherwise Kamidere will likely not work.\n\n"+
		...
		RawInfoModal("#openasar-confirm", "OpenAsar", "OpenAsar is an open-source alternative of Discord desktop's app.asar.\n"+
			"Kamidere is in no way affiliated with OpenAsar.\n"+
...
func loop() {
	...
	g.SingleWindow().
		...
		Layout(
			...
			g.Style().SetFontSize(20).To(
				g.Row(
					g.Label(Ternary(IsDevInstall, "Dev Install: ", "Kamidere will be downloaded to: ")+KamidereDirectory),
					g.Style().
						SetColor(g.StyleColorButton, DiscordBlue).
						SetStyle(g.StyleVarFramePadding, 4, 4).
						To(
							g.Button("Open Directory").OnClick(func() {
								g.OpenURL("file://" + path.Dir(KamidereDirectory))
							}),
						),
				),
				&CondWidget{!IsDevInstall, func() g.Widget {
					return g.Label("To customise this location, set the environment variable '" + EnvUserDataDir + "' and restart me").Wrapped(true)
				}, nil},
				g.Dummy(0, 10),
				g.Label("Kamidere Installer Version: "+buildinfo.InstallerTag+" ("+buildinfo.InstallerGitHash+")"+Ternary(IsSelfOutdated, " - OUTDATED", "")),
				g.Label("Local Kamidere Version: "+InstalledHash),
				&CondWidget{
					GithubError == nil,
					func() g.Widget {
						if IsDevInstall {
							return g.Label("Not updating Kamidere due to being in DevMode")
						}
						return g.Label("Latest Kamidere Version: " + LatestHash)
					}, func() g.Widget {
